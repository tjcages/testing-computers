...
export function Computers(props) {
  ...
  return (
    <group {...props} dispose={null}>
      ...
      {/* <VerticalScreenText frame="Object_209" panel="Object_210" y={5} position={[-1.43, 2.5, -1.8]} rotation={[0, 1, 0]} /> */}
...
function ScreenText({ invert, x = 0, y = 1.2, ...props }) {
  ...
  useFrame((state) => (textRef.current.position.x = x + Math.sin(rand + state.clock.elapsedTime / 12) * 16))
  return (
    <Screen {...props}>
...
/* Renders a monitor with some text */
function VerticalScreenText({ invert, x = 0, y = 1.2, ...props }) {
  const textRef = useRef()
  const cursiveRef = useRef()
  const rand = Math.random() * 10000
  useFrame((state) => {
    textRef.current.position.x = x + Math.sin(rand + state.clock.elapsedTime / 4) * 7
    cursiveRef.current.position.x = x + Math.sin(rand - state.clock.elapsedTime / 4) * 7
  })
  return (
    <Screen {...props}>
      <PerspectiveCamera makeDefault manual aspect={1 / 1} position={[0, 0, 15]} />
      ...
      <Text font="./fonts/Allegratta.ttf" characters="potencia" position={[x, y, 0]} ref={cursiveRef} fontSize={4} letterSpacing={-0.1} color={!invert ? 'black' : '#35c19f'}>
        POTENCIA
      </Text>
    </Screen>
  )
